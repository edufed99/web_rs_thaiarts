...
│   ├── app/
│   │   ├── private_main.py     the only ASGI entry point (no public routes)
│   │   ├── model_loader.py     ArtifactLoader singleton
│   │   ├── core/               config (RECSYS_* env vars) + exceptions
│   │   ├── schemas/            Pydantic v2 private-contract models (inference)
...
└── frontend/                   Next.js 14 + TypeScript Application Backend
    ...
    ├── tests/                  public-boundary node --test suites
    └── lib/                    api.ts (typed same-origin fetch), types.ts, lib/server/ (route logic)
```
...
application (`app.main`) was deleted, docs endpoints are disabled, and the
Internal Service Credential is required — an unset secret makes the
private contract unavailable (503) instead of weakening auth. The Next.js
Application Backend is the only caller, over the internal Docker network.
...
routes and its docs endpoints are disabled (issue #10).
