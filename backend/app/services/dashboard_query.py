...
The online evaluation is recomputed lazily by ``recompute_online_eval``
which is called from the recommendations router after each successful
admit. The pipeline is small enough (~50 ms on 10k recs) that we do it
on every request; if it becomes a bottleneck the recompute can be
moved to a TTL cache on the dashboard read path.
"""
...
# ---------------------------------------------------------------------------
# Online evaluation (recompute from live telemetry)
# ---------------------------------------------------------------------------
