"""Tests for ``services.user_query`` — DB-layer helpers for the auth slice.
...
def test_find_user_by_username_returns_none_when_empty(db_session):
    assert user_query.find_user_by_username("alice") is None


def test_create_user_then_find(db_session):
    u = user_query.create_user("alice", "$2b$12$abcdef", "Alice", is_admin=False)
    assert u is not None
    assert u.username == "alice"
    assert u.is_admin is False

    found = user_query.find_user_by_username("alice")
...
def test_create_user_rejects_duplicate(db_session):
    user_query.create_user("dup", "$2b$12$x")
    again = user_query.create_user("dup", "$2b$12$x")
    assert again is None


def test_set_last_login_stamps(db_session):
    u = user_query.create_user("alice", "$2b$12$aa")
    user_query.set_last_login(u.id)
    fresh = user_query.find_user_by_id(u.id)
    assert fresh is not None
    assert fresh.last_login_at is not None


def test_count_users(db_session):
    assert user_query.count_users() == 0
    user_query.create_user("a", "$2b$12$aa")
    user_query.create_user("b", "$2b$12$bb")
    assert user_query.count_users() == 2


def test_list_users_caps_limit(db_session):
    for i in range(3):
        user_query.create_user(f"u{i}", "$2b$12$aa")
    rows = user_query.list_users(limit=2)
    assert len(rows) == 2
...
def test_disabled_db_short_circuits(monkeypatch):
    ...
    assert user_query.find_user_by_id(1) is None
    assert user_query.create_user("any", "x") is None
    assert user_query.count_users() == 0
    assert user_query.list_users() == []
    # set_last_login is a no-op when DB is off.
    user_query.set_last_login(1)